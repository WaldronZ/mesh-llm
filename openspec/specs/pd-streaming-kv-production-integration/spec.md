...
## Purpose
Define the default-off production `pd-kv-stream/1` serving integration for
Skippy PD serving. The integration validates Mac router to PGX `serve-binary`
split-channel KV page export/import, final contiguous gate, trim/replay
bootstrap, and SSE decode for short and 4k foreground correctness scope.
Production performance readiness, 8k and longer validation, payload reduction,
timeout/cancel hardening, scheduler/concurrency, and UI work remain out of
